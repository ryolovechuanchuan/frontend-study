...
async function getApiData() {
    try {
        ...
        const users = (await response.json()); //加入介面限制每一個user的資料型態
        users.forEach((item) => {
        ...
        if (error instanceof Error)
            console.log(error.message); //if(error instanceof Error)  確認error的內容是Error的物件資料
    }
}
// getApiData();
async function getPostApi() {
    try {
        ...
        const postsdata = (await respone.json());
